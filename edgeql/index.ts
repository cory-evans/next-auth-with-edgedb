...
import edgeql from "./edgeql-js";
import { edgedb } from "./edgeql-js/imports";
...
export function EdgeDBAdapter(): Adapter {
  const c = edgedb.createClient();
  const e = edgeql;
  return {
    ...
    updateUser: async (data) => {
      ...
      const u = await getUserByID(c, e, row[0].id);
    ...
    linkAccount: async (data) => {
      const row = await e.insert(e.Account, data as any).run(c);
    ...
    updateSession: async (data) => {
      ...
      const u = getSessionByID(c, e, row[0].id);
